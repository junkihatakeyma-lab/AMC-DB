...
window.openAIConfirm = async function(bom) {
    if (!confirm('縺薙・BOM繝ｬ繧､繧｢繧ｦ繝医ｒ豁｣縺励＞繧ゅ・縺ｨ縺励※遒ｺ螳壹＠縺ｾ縺吶°・・)) return;
    try {
        const res = await fetch('/api/ai_confirm', {
            method: 'POST',
        ...
        if (data.status === 'ok') {
            alert('蜃ｦ逅・′螳御ｺ・＠縺ｾ縺励◆/繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆縲ゅΜ繝ｭ繝ｼ繝峨＠縺ｾ縺吶・);
            location.reload();
        } else {
            alert('蜃ｦ逅・′螳御ｺ・＠縺ｾ縺励◆/繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆縲ゅΜ繝ｭ繝ｼ繝峨＠縺ｾ縺吶・);
        }
...
window.registerProductName = async function(pCode) {
    const newName = prompt(`陬ｽ蜩∝錐繧貞､画峩縺励∪縺吶よ眠縺励＞蜷榊燕繧貞・蜉帙＠縺ｦ縺上□縺輔＞縲・(萓・ 驛ｨ蜩∫分蜿ｷ縲・{pCode}縲阪・豁｣蠑上↑陬ｽ蜩∝錐繧貞・蜉帙＠縺ｦ縺上□縺輔＞\n(萓・ ${pCode} 蝗ｳ:KF31-14-13)`);
    if (!newName) return;
    
    try {
        const res = await fetch('/api/register_product', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ product_code: pCode, name: newName })
        });
        const data = await res.json();
        if (data.status === 'ok') {
            alert('蜃ｦ逅・′螳御ｺ・＠縺ｾ縺励◆/繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆縲ゅΜ繝ｭ繝ｼ繝峨＠縺ｾ縺吶・);
...
window.updateHinmei = async function(reqNo, oldHinmei) {
    const newHinmei = prompt(`依頼No. ${reqNo} の品名を変更します。`, oldHinmei);
    if (newHinmei === null) return;
    try {
        const res = await fetch('/api/update_hinmei', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ request_no: reqNo, hinmei: newHinmei })
        });
        const data = await res.json();
        if (data.status === 'ok') {
            alert('更新しました、E);
...
window.addPartInput = function(val = '') {
    ...
    if (currentInputs.length >= 5) {
        alert('驛ｨ蜩∫分蜿ｷ縺ｮ讀懃ｴ｢遯薙・譛螟ｧ5縺､縺ｾ縺ｧ縺ｧ縺吶・);
        return;
    }
    ...
    if (list) {
        list.innerHTML = `
            <div class="part-input-row" style="display: flex; gap: 4px;">
                <input type="text" class="searchPartInput" placeholder="侁E IF000-304" style="flex: 1;">
            </div>
        `;
...
async function performSearch() {
    ...
    if (!q && !seiban && !req && !product && !part && !company) {
        document.getElementById('results').innerHTML = '<div class="empty-state">讀懃ｴ｢譚｡莉ｶ繧貞・蜉帙＠縺ｦ縺上□縺輔＞</div>';
        return;
    }
        ...
        if (list) {
            list.innerHTML = `
                <div class="part-input-row" style="display: flex; gap: 4px; flex: 1;">
                    <input type="text" class="searchPartInput" placeholder="侁E IF000-304" style="width: 100%;">
                </div>
            `;
            document.querySelector('.searchPartInput').addEventListener('input', () => {
                clearTimeout(searchTimeout);
...
function renderProductSearch(groups, tokens) {
    ...
    groups.forEach(group => {
        ...
        const pCode = p.product_code || '譛ｪ蛻・｡・;
        let titleText = pCode;
        if (p.name && p.name !== '未登録部品E && p.name !== '未登録製品E依頼書)') {
            titleText = p.name;
        ...
        const pAlias = p.alias ? `<p style="margin: 0.5rem 0 0 0; font-size: 0.95rem; color: #cbd5e1;">別吁E ${highlight(p.alias, tokens)}</p>` : '';
        
        let boxHtml = `
            <div class="product-box">
                <div class="product-header">
                    <h2 class="product-title">
                        ${isUnclassified ? '逃 譛ｪ蛻・｡槭・譖ｸ鬘・ : `逃 陬ｽ蜩・ ${highlight(titleText, tokens)}`}
                    ...
                    ${(p.name === '未登録部品E || p.name === '未登録製品E依頼書)') ? `<button class="btn btn-secondary" style="margin-left: 1rem; padding: 2px 8px; font-size: 0.85rem;" onclick="registerProductName('${escapeHtml(pCode).replace(/'/g, "\\\\'")}')">製品名を登録する</button>` : ''}
                    ${pAlias}
                </div>
        `;
                ...
                if (b.components && b.components.length > 0) {
                    ...
                    if (Array.isArray(b.components)) {
                        b.components.forEach(c => {
                            let isRedText = c.role === '縲千音險倥・襍､蟄励・;
                ...
                boxHtml += `
                    <div class="item-card">
                        ...
                        <div class="action-bar">
                            ...
                            ${b.sp_url ? `<a href="${b.sp_url}" target="_blank" class="btn btn-primary" style="background:#0078d4; border-color:#0078d4;">🌐 ブラウザで開く (Excel for Web)</a>` : (b.file ? `<a href="/${b.file.replace(/\\/g, '/').split('/').map(encodeURIComponent).join('/')}" target="_blank" class="btn btn-secondary">Excelを開ぁE/a>` : '')}
                            ${!b.layout_ok ? `<button class="btn btn-secondary" style="border:1px solid var(--danger);color:var(--danger)" onclick='openAIConfirm(${escapeHtml(JSON.stringify(b))})'>AI解析を確宁E/button>` : ''}
                        </div>
                    </div>
                `;
            });
        ...
        if (group.requests && group.requests.length > 0) {
            ...
            group.requests.forEach(r => {
                const isModified = r.file && (r.file.includes('、E) || r.file.includes('ÁE) || r.file.includes('修正'));
                boxHtml += `
                    <div class="item-card ${isModified ? 'highlight-modified' : ''}">
                        ...
                        <div class="table-wrap">
                            <table>
                                ...
                                <tr><th>数釁E/th><td>${escapeHtml(r.qty)}</td></tr>
                                <tr><th>生地色</th><td>${highlight(r.kiji, tokens)}</td></tr>
                                <tr><th>用送E/th><td>${highlight(r.yoto, tokens)}</td></tr>
                                <tr><th>納期</th><td>${escapeHtml(r.noki || r.noki_raw)}</td></tr>
                                <tr><th>出荷允E/th><td>${highlight(r.dest, tokens)}</td></tr>
                                <tr><th>規格</th><td><pre style="margin:0;font-family:inherit;white-space:pre-wrap">${highlight(r.spec, tokens)}</pre></td></tr>
                                <tr><th>備老E/th><td><pre style="margin:0;font-family:inherit;white-space:pre-wrap">${highlight(r.biko, tokens)}</pre></td></tr>
                            </table>
                        </div>
                        <div class="action-bar">
                            ...
                            ${r.is_handwritten ? `<button class="btn btn-secondary" onclick="updateHinmei('${escapeHtml(r.request_no)}', '${escapeHtml(r.hinmei)}')">品名を編雁E/button>` : ''}
                        </div>
                    </div>
                `;
            });
        ...
        if (group.inspections && group.inspections.length > 0) {
             boxHtml += `
                <div class="section">
                    <div class="section-header" onclick="this.parentElement.classList.toggle('open')">
                        <span>✁E検査証 (${group.inspections.length}件)</span>
                    </div>
                    <div class="section-content">
                        <div class="section-inner">
            `;
                ...
                <div class="group-content">
                    <div class="item-card ${isModified ? 'highlight-modified' : ''}" style="border: 2px solid var(--primary); margin-bottom: 1rem;">
                        ...
                        <div class="table-wrap" style="margin-bottom:0.5rem">
                            <table>
                                <tr><th>品名</th><td>${highlight(r.hinmei, tokens)}</td><th>数釁E/th><td>${highlight(r.qty, tokens)}</td></tr>
                                <tr><th>生地</th><td>${highlight(r.kiji, tokens)}</td><th>用送E/th><td>${highlight(r.yoto, tokens)}</td></tr>
                                <tr><th>仕槁E/th><td colspan="3">${highlight(r.spec, tokens)}</td></tr>
                                <tr><th>備老E/th><td colspan="3">${highlight(r.biko, tokens)}</td></tr>
                            </table>
                        </div>
                        <div class="action-bar">
                            <button class="btn btn-primary" onclick='openPreview(${escapeHtml(JSON.stringify(r.previews))}, ${escapeHtml(JSON.stringify(r.file))})'>プレビュー</button>
...
function renderGlobalSearch(groups, tokens, state) {
    ...
    if (showPhoto && allPhotos.length > 0) {
        ...
        allPhotos.forEach(p => {
            ...
            const pCode = p._product ? p._product.product_code : '譛ｪ蛻・｡・;
            html += `
                <div class="item-card" style="flex:1; min-width:300px">
                    <h4>${escapeHtml(p.file_path.replace(/\\/g, '/').split('/').pop())}</h4>
                    <p style="font-size:0.85rem; color:var(--text-muted); margin: 0 0 0.5rem 0;">関連製品E ${escapeHtml(pCode)}${escapeHtml(pName)}</p>
    ...
    if (showDrawing && allDrawings.length > 0) {
        ...
        allDrawings.forEach(d => {
            ...
            const pCode = d._product ? d._product.product_code : '譛ｪ蛻・｡・;
            html += `
                <div class="item-card" style="flex:1; min-width:250px">
                    <h4>${escapeHtml(d.file_path.replace(/\\/g, '/').split('/').pop())}</h4>
                    <p style="font-size:0.85rem; color:var(--text-muted); margin: 0 0 0.5rem 0;">関連製品E ${escapeHtml(pCode)}${escapeHtml(pName)}</p>
                    <div class="action-bar">
                        ${d.previews && d.previews.length > 0 && d.previews[0] !== 'previews/dummy_0.png' ? `<button class="btn btn-primary" onclick='openPreview(${escapeHtml(JSON.stringify(d.previews))}, ${escapeHtml(JSON.stringify(d.file_path))})'>プレビュー</button>` : ''}
                        ${d.file_path ? `<a href="/${d.file_path.replace(/\\/g, '/').split('/').map(encodeURIComponent).join('/')}" target="_blank" class="btn btn-secondary">ダウンローチE/ 開く</a>` : ''}
                    </div>
                </div>
            `;
        });
