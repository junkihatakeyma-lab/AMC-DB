...
window.openAIConfirm = async function(bom) {
    if (!confirm('このBOMレイアウトを正しいものとして確定しますか？')) return;
    try {
        ...
        if (data.status === 'ok') {
            alert('処理が完了しました/エラーが発生しました。リロードします。');
            location.reload();
        } else {
            alert('処理が完了しました/エラーが発生しました。リロードします。');
        }
...
window.registerProductName = async function(pCode) {
    const newName = prompt(`製品名を変更します。新しい名前を入力してください。 (例: 部品番号「${pCode}」の正式な製品名を入力してください\n(例: ${pCode} 図:KF31-14-13)`);
    if (!newName) return;
...
function renderGlobalSearch(groups, tokens, state) {
    ...
    statsContainer.textContent = `全体で ${totalHits} 件ヒットしました${totalHits > 500 ? ' (結果が多すぎるため、各カテゴリ上位100件のみ表示しています)' : ''}`;
    ...
    if (showBom && allBoms.length > 0) {
        html += `<div class="flat-section"><h3>📋 BOM一覧 (${allBoms.length}件表示 / 全${origBomsLen}件中)</h3>`;
        allBoms.forEach(b => {
            let compsHtml = '';
            if (b.components && b.components.length > 0) {
    ...
    if (showReq && allReqs.length > 0) {
        html += `<div class="flat-section"><h3>📄 加工依頼書一覧 (${allReqs.length}件表示 / 全${origReqsLen}件中)</h3>`;
        allReqs.forEach(r => {
            const isModified = r.file && (r.file.includes('〇') || r.file.includes('×') || r.file.includes('修正'));
            html += `
                <div class="item-card ${isModified ? 'highlight-modified' : ''}">
    ...
    if (showInsp && allInsps.length > 0) {
        html += `<div class="flat-section"><h3>🔍 検査証一覧 (${allInsps.length}件表示 / 全${origInspsLen}件中)</h3><div style="display:flex; flex-wrap:wrap; gap:1rem">`;
        allInsps.forEach(i => {
            html += `
    ...
    if (showPhoto && allPhotos.length > 0) {
        html += `<div class="flat-section"><h3>📷 写真・その他一覧 (${allPhotos.length}件表示 / 全${origPhotosLen}件中)</h3><div style="display:flex; flex-wrap:wrap; gap:1rem">`;
        allPhotos.forEach(p => {
    ...
    if (showDrawing && allDrawings.length > 0) {
        html += `<div class="flat-section"><h3>📐 図面一覧 (${allDrawings.length}件表示 / 全${origDrawingsLen}件中)</h3><div style="display:flex; flex-wrap:wrap; gap:1rem">`;
